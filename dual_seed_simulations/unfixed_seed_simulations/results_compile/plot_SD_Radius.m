...
axis on;
SNR = [0 3 6 9 12];
data_mean = reshape(mean(sd1_20hz_compile,1),5,5).';
data_std = reshape(std(sd1_20hz_compile,0,1),5,5).'/sqrt(30);
...
ylabel({'r = 3mm','SD (mm)'},'FontSize',label_FontSize_y);
...
hold on;
axis on;
data_mean = reshape(mean(sd1_45hz_compile,1),5,5).';
data_std = reshape(std(sd1_45hz_compile,0,1),5,5).'/sqrt(30);
GO = bar(SNR,data_mean,1,'EdgeColor','none');
% Calculate the number of groups and number of bars in each group
[ngroups,nbars] = size(data_mean);
% Get the x coordinate of the bars
...
hold on;
axis on;
data_mean = reshape(mean(sd2_20hz_compile,1),5,5).';
data_std = reshape(std(sd2_20hz_compile,0,1),5,5).'/sqrt(30);
GO = bar(SNR,data_mean,1,'EdgeColor','none');
% Calculate the number of groups and number of bars in each group
[ngroups,nbars] = size(data_mean);
% Get the x coordinate of the bars
...
hold off;
xlabel({'SNR_s_i_m_u (dB)'},'FontSize',label_FontSize);
ylabel({'r = 6mm','SD (mm)'},'FontSize',label_FontSize_y);
title({'20Hz'},'FontSize',title_FontSize);
set(gca,'Layer','top');
...
hold on;
axis on;
data_mean = reshape(mean(sd2_45hz_compile,1),5,5).';
data_std = reshape(std(sd2_45hz_compile,0,1),5,5).'/sqrt(30);
GO = bar(SNR,data_mean,1,'EdgeColor','none');
% Calculate the number of groups and number of bars in each group
[ngroups,nbars] = size(data_mean);
% Get the x coordinate of the bars
